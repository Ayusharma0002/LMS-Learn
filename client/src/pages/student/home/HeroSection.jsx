...
export default function HeroSection() {
  const [boxes, setBoxes] = useState([]);
  const [hover, setHover] = useState(false); // State to track hover
  const [hoveredBoxId, setHoveredBoxId] = useState(null); // Track hovered box
  const [clickedBoxId, setClickedBoxId] = useState(null); // Track clicked box
        ...
        {boxes.map((box) => (
          <motion.div
            ...
            style={{
              ...
              left: box.left,
              border: 
                box.id === hoveredBoxId || box.id === clickedBoxId 
                  ? "2px solid red" 
                  : "none", // Apply red border conditionally
            }}
            // className="absolute text-xl font-bold text-gray-50 bg-opacity-50 bg-gray-300 p-4 rounded-lg pointer-events-none"
            className="absolute text-xl font-bold text-gray-50 bg-opacity-50 bg-gray-300 p-4 rounded-lg  "
            initial={{ opacity: 0 }}
            animate={{
              opacity: 1,
              top: box.top,
            ...
            onMouseEnter={() => setHoveredBoxId(box.id)} // Set hovered box ID
            onMouseLeave={() => setHoveredBoxId(null)} // Reset hovered box ID
            onClick={() => setClickedBoxId(box.id)} // Set clicked box ID
          >
            <span>Moving Box</span>
          </motion.div>
        ))}
...
//                   : "none", // Apply red border conditionally
//             }}
//             className="absolute text-xl font-bold text-gray-50 bg-opacity-50 bg-gray-300 p-4 rounded-lg pointer-events-auto cursor-pointer"
//             initial={{ opacity: 0 }}
//             animate={{
//               opacity: 1,
//               top: box.top,
...
//           </h1>
//           <p className="mt-6 text-lg md:text-xl text-gray-700 leading-relaxed">
//             Shape your career with expert-led courses, industry-recognized
//             certifications.
//           </p>
//           <motion.div
//             initial={{ opacity: 0, y: 20 }}
