...
//       if (response.success) {
//         const options = {
//           key: "rzp_test_RaMCRkTVomuV9r", // Razorpay key
...
//             // Redirect to another page (e.g., course access or receipt page)
//             // navigate(`/course-access/${studentViewCourseDetails?._id}`);
//           },
//           prefill: {
//             name: auth?.user?.userName,
//             email: auth?.user?.userEmail,
//             // contact: "1234567890", // Optional contact number
//           },
//           notes: {
//             courseId: studentViewCourseDetails?._id,
//           },
...
import { AuthContext } from "@/context/auth-context";
import { StudentContext } from "@/context/student-context";
import { fetchStudentViewCourseDetailsService, createPaymentService, captureAndFinalizePaymentService } from "@/services";
import { CheckCircle, Globe, Lock, PlayCircle } from "lucide-react";
import { useContext, useEffect, useState } from "react";
...
function StudentViewCourseDetailsPage() {
  ...
  async function fetchStudentViewCourseDetails() {
    ...
    if (response?.success) {
      ...
      setStudentViewCourseDetails(response?.data);
      // setCoursePurchaseId(response?.CoursePurchased);
      setLoadingState(false);
    } else {
      setStudentViewCourseDetails(null);
      // setCoursePurchaseId(null);
  ...
  async function handleCreatePayment() {
    ...
    try {
      const response = await createPaymentService(paymentPayload);
      console.log("Payment Service Response: ", paymentPayload);

      if (response.success) {
        const options = {
          key: "rzp_test_RaMCRkTVomuV9r", // Replace with your Razorpay Key ID
          ...
          handler: async function (paymentResult) {
            ...
            // Capture payment and finalize the order in the backend
            const captureResponse = await captureAndFinalizePaymentService(
              paymentResult.razorpay_payment_id,
              paymentResult.razorpay_order_id,
              paymentResult.razorpay_signature,
              response.data.orderId // Backend-generated order ID
            );
          ...
          modal: {
            ondismiss: function () {
              ...
              navigate(`/payment-cancel/${studentViewCourseDetails?._id}`);
            },
          },
        };
